...
export default async function Start(bot: Telegraf<Context<Update>>, db: any) {
  bot.start(async (ctx) => {
    ...
    if (await db.get(ctx.chat.id)('email') && await db.get(ctx.chat.id)('email') !== '') {
      ...
      await ctx.reply(`Привет! Меня зовут Диана🧜🏻‍♀️\n
<b>Ты здесь не просто так,</b> <i>я знаю, чего ты хочешь.</i> <b>Хочешь увидеть мои горячие фотографии, эксклюзивные видео и всё то, что не покажут в свободном доступе? Тогда ты попал по адресу😸🔥\n
Здесь, в моём закрытом Телеграм-канале, я делюсь самым откровенным и сексуальным контентом:</b> <u>фото, видео, кружочки и истории, от которых твоё сердце точно не останется равнодушным</u>🫶🤤🍌\n
<i>Ты готов? Тогда выбирай свой уровень доступа и погружайся в мой мир без запретов</i>😘\n
<b>Это не просто контент — это то, что ты будешь вспоминать снова и снова</b>🫶
Ну что, встал? Поскорее вводи свой <i>e-mail</i>, чтобы присоедениться к нам!`);
